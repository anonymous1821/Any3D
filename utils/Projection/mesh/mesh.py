...
class Mesh:
    ...
    def load(cls, path=None, remesh=False, resize=True, renormal=True, retex=False, auto_uv=False, front_dir='+z', **kwargs):
        ...
        log.info(f"cls.load_obj took {toc - tic:0.4f} seconds")

        log.info(f"[Mesh loading] v: {mesh.v.shape}, f: {mesh.f.shape}")
        ...
        toc = time.perf_counter()
        log.info(f"Total mesh loading took {toc - tic:0.4f} seconds")
    ...
    def load_obj(cls, path, albedo_path=None, device=None):
        ...
        if mesh.v.shape[1] == 6:
            ...
            mesh.vc = mesh.v[:, 3:]
            mesh.v = mesh.v[:, :3]
            log.info(f"[load_obj] use vertex color: {mesh.vc.shape}")
        ...
        if not use_vertex_color:
            ...
            if albedo_path is None or not os.path.exists(albedo_path):
                ...
                log.info(f"[load_obj] load texture: {albedo.shape}")
    ...
    def load_trimesh(cls, path, device=None):
        ...
        if _mesh.visual.kind == 'vertex':
            ...
            log.info(f"[load_trimesh] load texture: {texture.shape}")
        else:
            texture = np.ones((1024, 1024, 3), dtype=np.float32) * np.array([0.5, 0.5, 0.5])
